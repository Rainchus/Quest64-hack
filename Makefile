...
genMain:
	...
	$(file >> $(OUTPUT_FILE),.include "asm/Quest64.asm" // Include Quest64.asm to tell armips' linker where to find the game's function(s))
	$(file >> $(OUTPUT_FILE),.headersize 0x7FFFF400)
	$(file >> $(OUTPUT_FILE),.org 0x80410000)
	$(file >> $(OUTPUT_FILE),.include "asm/boot.asm")
